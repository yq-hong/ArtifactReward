...
class GDino:
    def __init__(self, args):
        ...
        # spatial info
        self.vocab_spatial = ['on side of', 'next to', 'near', 'on the left of', 'on the right of', 'on the bottom of',
                              'on the top of', 'on top of', 'right of', 'left of', 'below', 'above']  # locality words
    ...
    def get_grounding_output(self, image, caption, box_threshold, text_threshold=None, with_logits=True,
                             token_spans=None):
        ...
        if token_spans is None:
            ...
            # given-phrase mode
            positive_maps = create_positive_map_from_span(self.model.tokenizer(caption),
                                                          token_span=token_spans).to(image.device)  # n_phrase, 256
    ...
    def make_prompt(self, nouns):
        ...
        for noun in nouns:
            ...
            if len(n_split) == 1:
                ...
                token_spans.append([[pointer, pointer + beg_len],
    ...
    def get_spatial_score(self, boxes_filt, pred_phrases, all_logits, nouns, spatial_info):
        ...
        if score == 0.4:
            ...
            obj1_nms_boxes, obj2_nms_boxes = self.get_nms_boxes(boxes_filt, pred_phrases, all_logits, nouns,
                                                                spatial_info)
    ...
    def get_numeracy_score(self, boxes_filt, pred_phrases, all_logits, nouns, numeracy_info):
        ...
        for idx, num_item in enumerate(numeracy_info):
            ...
            detected_boxes_index = nms(torch.stack(detected_boxes).to(self.device),
                                       torch.stack(detected_scores).to(self.device), 0.5)
    ...
    def __call__(self, prompts, images, **kwargs):
        ...
        for idx, image in enumerate(images):
            ...
            text_prompt, token_spans = (kwargs['det_prompt'][idx]['text_prompt'],
                                        kwargs['det_prompt'][idx]['token_spans'])

            boxes_filt, pred_phrases, all_logits = self.get_grounding_output(image, text_prompt, self.box_threshold,
                                                                             self.text_threshold,
            ...
            if kwargs['task_type'][idx] == 'spatial':
                try:
                    score = self.get_spatial_score(boxes_filt, pred_phrases, all_logits, kwargs['nouns'][idx],
                                                   kwargs['spatial_info'][idx])
                except:
                ...
                score = self.get_numeracy_score(boxes_filt, pred_phrases, all_logits, kwargs['nouns'][idx],
                                                kwargs['numeracy_info'][idx])
            else:
